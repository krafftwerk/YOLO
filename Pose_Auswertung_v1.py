...
# Ausgabe
OUTPUT_DIR = Path("pose_evaluation_v1")
...
def heatspreader_center(kpts, visibility=None):
    ...
    Fuer GT werden nur gueltige Punkte verwendet; fuer Prediction existieren alle Punkte.
    Mindestens 2 gueltige Aussenpunkte sind erforderlich.
    """
    indices = np.array([0, 1, 2, 3], dtype=int)

    if visibility is not None:
        vis = np.asarray(visibility)
        indices = indices[vis[indices] >= MIN_GT_VISIBILITY_FOR_EVAL]
...
def heatspreader_angle_360(kpts, visibility=None):
    ...
    if visibility is not None:
        vis = np.asarray(visibility)
        left_idx = left_idx[vis[left_idx] >= MIN_GT_VISIBILITY_FOR_EVAL]
        right_idx = right_idx[vis[right_idx] >= MIN_GT_VISIBILITY_FOR_EVAL]
...
def slot_center(kpts, visibility=None):
    """Mittelwert aller auswertbaren Slot-Keypoints."""
    pts = np.asarray(kpts, dtype=float)

    if visibility is None:
        indices = np.arange(len(pts))
    else:
        indices = valid_gt_indices(visibility)
...
def evaluate_matches(gt_by_image, pred_by_image, names):
    ...
    for image_key in image_keys:
        ...
        for gi, pi, iou in matches:
            ...
            gt_pose = derive_pose(gt["cls"], names, gt["kpts"], gt["visibility"])
            pred_pose = derive_pose(pred["cls"], names, pred["kpts"], None)
...
def write_legend(path):
    ...
    text = f"""YOLO POSE AUSWERTUNG V1 - LEGENDE
=================================
...
----------------
center_error_px:
Euklidischer Abstand zwischen dem aus GT-Keypoints und Prediction-Keypoints jeweils
identisch berechneten Mittelpunkt.
